...
#End Region
    ...
    Private Sub btn_Archive_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btn_Archive.Click
        If cmbJobs.Text = "" Then
            ...
            cmbJobName.Focus()
        Else
            If MessageBox.Show("Are you sure you want to delete job " & cmbJobs.Text & " ? ", "WARNING!", MessageBoxButtons.YesNo, MessageBoxIcon.Warning) = DialogResult.Yes Then

                copyJobCuts(cmbJobs.Text)
                copyJobInv(cmbJobs.Text)
                deleteInvoice(cmbJobs.Text)
                copyJob(cmbJobs.Text)
                delJobCuts(cmbJobs.Text)
                deleteJob(cmbJobs.Text)
                populate_cmb_jobs()
    ...
    Private Sub copyJobCuts(ByVal inJob As String)
        ...
        Dim sqlIns = "INSERT INTO ACuttingSheet " & _
